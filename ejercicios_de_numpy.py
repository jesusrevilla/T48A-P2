...
Original file is located at
    https://colab.research.google.com/drive/1JzlEcsE12kWF_1XF5UO6YmFzvbb2a6Gv
...
import numpy as np
#Diego Adolfo Guerrero Gonzalez - 177156
...
def rand_float():
    ...
    Para poder mantener la generación de números aleatorios fija en los ejemplos,
    se utiliza un seed en la documentación pero no en la implementación.
    ...
    Examples
    --------
    >>> np.random.seed(10)  # Solo para la prueba
    >>> rand_float()
...
def subtract_arrays(arr1, arr2):
    ...
    -------
    numpy.ndarray
        Resultado de restar arr1 a arr2, es decir, `arr2 - arr1`.
...
def multiply_arrays(arr1, arr2):
    ...
    -------
    numpy.ndarray
        Resultado de la multiplicación elemento por elemento de `arr1` y `arr2`.
...
def divide_arrays(arr1, arr2):
    ...
    -------
    numpy.ndarray
        Resultado de la división elemento por elemento de `arr2 / arr1`.

    Parameters
    ----------
    arr1: numpy.ndarray
        El divisor (no debe contener ceros).
    arr2: numpy.ndarray
        El dividendo.
    ...
    '''
    assert arr1.shape == arr2.shape, 'Los arreglos deben tener el mismo tamaño'
    assert np.all(arr2 != 0), 'No se puede dividir por cero'
    return arr1 / arr2  # Se invirtió el orden de los parámetros
...
def stats(arr):
    ...
    en un tuple con las siguientes posiciones: (media, mediana, desviación estándar).

    Returns
    -------
    tuple
        Tuple con las siguientes posiciones: (media, mediana, desviación estándar).

    Parameters
    ----------
    arr: numpy.ndarray
        Arreglo de numpy con los números de 1 a 5.
...
def first_5(arr):
    '''Regresa los primeros 5 elementos de un arreglo que contiene 10 números
...
def last_3(arr):
    '''Regresa los últimos 3 elementos de un arreglo de numpy que contiene 10
...
def indices_2_4_6(arr):
    '''Regresa los elementos en los índices 2, 4 y 6 de un arreglo de numpy que contiene
    10 números enteros aleatorios entre 0 y 100.

    Returns
    -------
    numpy.ndarray
        Un arreglo con los elementos en los índices 2, 4 y 6 de `arr`.
...
def greater_50(arr):
    '''Regresa los elementos del arreglo que contiene 10 números enteros
    aleatorios entre 0 y 100 que son mayores a 50.

    Returns
    -------
    numpy.ndarray
        Arreglo con los elementos de `arr` que son mayores a 50.
...
def less_7(arr):
    '''Regresa los elementos del arreglo que contiene 10 números enteros
    aleatorios entre 0 y 100 que son menores o iguales a 7.

    Returns
    -------
    numpy.ndarray
        Arreglo con los elementos de `arr` que son menores o iguales a 7.
...
def add_broadcast(arr1, arr2):
    ...
    -------
    numpy.ndarray
        Arreglo resultante de la suma de `arr1` y `arr2` con forma (2, 3).
    '''
    return arr1 + arr2
...
def subtract_broadcast(arr1, arr2):
    ...
    -------
    numpy.ndarray
        Resultado de la resta entre `arr1` y `arr2` tras ser transpuesto.
    '''
    return arr1 - arr2.T
...
def multiply_broadcast(arr1, arr2):
    ...
    -------
    numpy.ndarray
        Resultado de la multiplicación de `arr1` y `arr2`.
    '''
...
def divide_broadcast(arr1, arr2):
    ...
    -------
    numpy.ndarray
        Resultado de la división de `arr1` entre `arr2` con *broadcasting*.
    '''
    return arr1 / arr2
...
def element_wise_product(arr1, arr2):
    ...
    -------
    numpy.ndarray
        Resultado de la multiplicación elemento por elemento.
    '''
    return arr1 * arr2
...
def rainfall_data(rainfall):
    ...
    '''
    cities_with_rain_above_100 = np.where(np.any(rainfall[:, 1:] > 100, axis=1))[0]  # Ignorar primera columna si es ID
    print("Índices de las ciudades con más de 100 mm de lluvia:", list(cities_with_rain_above_100))
...
def image_thresholding(image, threshold=128):
    ...
    ----------
    image: numpy.ndarray
        Arreglo 2D de numpy que representa una imagen en escala de grises.
    threshold: int, optional
        Umbral para convertir la imagen a blanco y negro (por defecto 128).
